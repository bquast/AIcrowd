...
10. Log into the psql server again using `sudo -u postgres psql` and run `rake db:migrate` to migrate schema to our database then run `rake db:seed` to seed our database created using postgres. Then logout again (`\q`).
